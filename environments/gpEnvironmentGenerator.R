...
dimensions <- 3
xLength <- 10  #the number of points along each input dimension
lambda <- 1  #length scale, larger lambdas create smoother environments
n_envs <- 1 #number of environments to generate
...
grid <- expand.grid(replicate(dimensions, seq(xLength), simplify=FALSE))
